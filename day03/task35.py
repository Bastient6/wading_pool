...
language = ["anglais", "francais", "espagnol", "portuguais", "italien", "allemand", "néerlandais", "suédois", "polonais", "turc", "espéranto"]
def growing():
    max = 0
    index_max = 0
    new_tab_index = [0 for x in range (26)]
    new_tab_letter = [""]
    index_new_tab = 0
...
def choose_language(new_tab_letter):
    score = [0 for x in range (11)]
    max = 0
    max_index = 0
    for i in range (len(new_tab_letter) - 1):
        index = 0
        for j in LANGUAGE_PROFILES.items():
            strength = 0
            for e in j[1].keys():
                if new_tab_letter[i] == e:
                    score[index] += (26 - strength) * (26 - i)
                strength += 1
            index += 1
...
def moyenne_letter(string):
    for i in range (0, len(tab_letter)):
        tab_index[i] = "{:.2f}".format(tab_index[i] / len(string) * 100)
...
def language_detector():
    task11.print_string("ecris une phrase")
    string = input()
    string = task21.to_lowercase(string)
    count_letter(string)
    moyenne_letter(string)
    new_tab_letter = growing()
    index = choose_language(new_tab_letter)
    task11.print_string("la langue est :" + language[index])
